  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "chemistry",
              fluidRow(
                box(title = "Select Location(s)", status = "info", solidHeader = TRUE, width = 4,
                    pickerInput("location_filter", "Choose Location(s):",
                                choices = NULL,
                                multiple = TRUE,
                                options = list(`actions-box` = TRUE, `live-search` = TRUE))
                ),
                box(title = "Piper Plot", status = "primary", solidHeader = TRUE, width = 8,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$load_data, {
    ...
    tryCatch({
      showNotification("Custom GPKG upload not yet handled in this simplified version", type = "warning")
    }, error = function(e) {
